function [] = vary_stim_hrir_sweep(wall_xdim_str, wall_ydim_str, wall_zdim_str, wall_material_str, floor_material_str, cieling_material_str, head_x_str, head_y_str, head_z_str, speaker_dist_str)
    ...
    head_azimuth = 0;
    %set postion so that speak never touches room wall given the speaker is 1 meter
    %from the listener
    ...
    new_dir = sprintf('./Expanded_HRIRdist%d-5deg_elev_az_room%dx%dy%dz_materials%dwall%dfloor%dciel', round(100 * speaker_dist), wall_xdim, wall_ydim, wall_zdim, wall_material, floor_material, cieling_material);
