...
def room_impulse_hrtf(
    ...
    """
    meas_delay = (np.sqrt(np.sum(np.square(src_loc - head_center))) / c_snd) * np.ones(
        len(meas_files)
    )
...
def get_brir(
    ...
    assert (
        head_azim <= 135
    ), "head_azim > 135° causes unexpected behavior (recommended range: [0, 90])"
    room_materials = np.array(room_materials)
    assert room_materials.shape == (
        6,
    ), "room_materials shape: [wall_x0, wall_x, wall_y0, wall_y, floor, ceiling]"
    room_dim_xyz = np.array(room_dim_xyz)
    assert room_dim_xyz.shape == (
        3,
    ), "room_dim_xyz shape: [x_len (length), y_len (width), z_len (height)]"
    ...
    h_out, lead_zeros = room_impulse_hrtf(
        ...
        head_azimuth=-head_azim,  # convention is positive azimuth = clockwise
        meas_locs=meas_locs,
