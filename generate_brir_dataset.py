...
def get_display_str(itr, n_itr, n_skip=0, t_start=None):
    ...
    disp_str += " mem: {:06.3f} GB |".format(
        resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024 / 1024
    )
    if t_start is not None:
        time_per_signal = (time.time() - t_start) / (
            itr + 1 - n_skip
        )  # Seconds per signal
        time_remaining = (
            (n_itr - itr) * time_per_signal / 60.0
...
def main(df, fn, processes=8):
    ...
    sr = df.iloc[0].sr
    dur = df.iloc[0].dur
    hrtf_locs, hrtf_firs, hrtf_sr = simulator.load_kemar_hrtfs(
        npz_filename="kemar_hrtfs/hrtfs.npz"
    )
...
if __name__ == "__main__":
    ...
    fn_manifest_brir = "/om2/scratch/Mon/msaddler/brir/v00/manifest_brir.pdpkl"
    # tmp = "/om2/user/msaddler/spatial_audio_pipeline/assets/brir"
    # fn_manifest_brir = f"{tmp}/v00/manifest_brir.pdpkl"
    # fn_manifest_brir = f"{tmp}/mit_bldg46room1004/manifest_brir.pdpkl"
    # fn_manifest_brir = f"{tmp}/eval/manifest_brir.pdpkl"
    # fn_manifest_brir = f"{tmp}/speech_in_noise_in_reverb/manifest_brir.pdpkl"
    ...
    fn = os.path.join(
        os.path.dirname(fn_manifest_brir), "room{:04.0f}.hdf5".format(index_room)
    )
