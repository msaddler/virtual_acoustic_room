...
def sample_room_parameters(
    ...
    x_len = np.exp(
        np.random.uniform(
            low=np.log(range_room_x[0]), high=np.log(range_room_x[1]), size=None
        )
    )
    y_len = np.exp(
        np.random.uniform(
            low=np.log(range_room_y[0]), high=np.log(range_room_y[1]), size=None
        )
    )
    z_len = np.exp(
        np.random.uniform(
            low=np.log(range_room_z[0]), high=np.log(range_room_z[1]), size=None
        )
    )
    if is_outdoor:
        ...
        material_wall = np.random.choice(
            [-1, ANECHOIC], size=4, replace=True, p=[p_outdoor_wall, 1 - p_outdoor_wall]
        )
        IDX_outdoor_wall = material_wall < 0
        material_wall[IDX_outdoor_wall] = np.random.choice(
            list_material_outdoor_wall, size=IDX_outdoor_wall.sum(), replace=True
        )
    else:
        ...
        material_wall = np.random.choice(
            list_material_indoor_wall, size=n_unique_wall_materials, replace=False
        )
        if len(material_wall < 4):
            material_wall = np.concatenate(
                [
                    material_wall,
                    np.random.choice(
                        material_wall, size=4 - len(material_wall), replace=True
                    ),
                ]
            )
    room_parameters = {
...
def sample_head_parameters(
    ...
    max_z = min(
        range_head_z[1],
        room_dim_xyz[2] - buffer * np.sin(np.deg2rad(range_src_elev[1])),
    )
    assert (min_z <= max_z) and (
        min_z >= 0
    ), "invalid range of z values for head position"
    ...
    if verbose:
        print("[sample_head_parameters]")
        print(
            f"|__ head_pos_x sampled uniformly from {[buffer, room_dim_xyz[0]-buffer]})"
        )
        print(
            f"|__ head_pos_y sampled uniformly from {[buffer, room_dim_xyz[1]-buffer]})"
        )
...
def distance_to_wall(room_dim_xyz, head_pos_xyz, head_azim, src_azim, src_elev):
    ...
    if quadrant == 1:
        rx = (room_dim_xyz[0] - head_pos_xyz[0]) / (
            np.cos(np.deg2rad(azim)) * np.cos(np.deg2rad(elev))
        )
        ry = (room_dim_xyz[1] - head_pos_xyz[1]) / (
    ...
    for src_dist in list_src_dst:
        for src_azim in list_src_azim:
            for src_elev in list_src_elev:
                ...
                if strict:
                    assert simulator.is_valid_position(
                        head_pos_xyz, dfi_room.room_dim_xyz, buffer=0
                    )
                    assert simulator.is_valid_position(
                        src_pos_xyz, dfi_room.room_dim_xyz, buffer=0
                    )
                else:
                    if not simulator.is_valid_position(
                        head_pos_xyz, dfi_room.room_dim_xyz, buffer=0
                    ):
                        msg = f"WARNING: invalid head_pos_xyz={head_pos_xyz}"
                        msg += f" for room_dim_xyz={dfi_room.room_dim_xyz}"
                        print(msg)
                    if not simulator.is_valid_position(
                        src_pos_xyz, dfi_room.room_dim_xyz, buffer=0
                    ):
                        msg = f"WARNING: invalid src_pos_xyz={src_pos_xyz}"
                        msg += f" for room_dim_xyz={dfi_room.room_dim_xyz}"
                        print(msg)
                d = {
...
def main(
    ...
    for index_room in range(n_room):
        ...
        head_parameters = sample_head_parameters(
            room_dim_xyz=room_parameters["room_dim_xyz"], verbose=False
        )
    ...
    print(
        f"Wrote `df_room` ({len(df_room)} rooms / head positions):\n{fn_manifest_room}"
    )
...
def mit_46_1004(
    ...
    However -- because head azimuth values outside of [0, 90] caused bugs in
    the MATLAB room simulator, the head azimuth is fixed at 0 degrees (relative
    to the +x axis) and the entire room is rotated instead. Thus the four rooms
    ...
    Room materials: 9 = 'Fiberglass wall treatment,1 in'; 2 = 'Concrete, painted'.
    """
    list_room_dim_xyz = [
        (4.66, 5.90, 2.48),
        (5.90, 4.66, 2.48),
...
def eval_rooms(
    ...
    list_df_room = [
        ...
        {
            # Symmetric room with similar materials to speaker array room
            # (all "Fiberglass wall treatment, 1 in")
            "head_azim": 0,
            "head_pos_xyz": (2.5, 2.5, 2.0),
            "room_dim_xyz": (5.0, 5.0, 4.0),
